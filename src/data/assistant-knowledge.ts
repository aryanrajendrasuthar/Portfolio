export const assistantKnowledge = {
  intro:
    "Aryan Suthar is a backend-focused software engineer with 4+ years of experience building distributed, cloud-native applications and AI-integrated data platforms. He is currently a Software Engineer at Avnet in Arizona, building backend microservices and contributing to a Kafka-based event-driven pipeline, and is completing his MS in Software Engineering at Arizona State University (May 2026).",
  ...
  experience: {
    avnet:
      "At Avnet (Dec 2025 – Present), Aryan builds backend microservices in Python and Flask across 7+ services for data ingestion and core business logic, maintains transactional data in SQL Server, and built a GraphQL aggregation layer unifying SQL Server and MongoDB that cut page load times by 60%. He integrated AWS SQS/S3/Lambda for messaging, storage, and serverless data validation (~20,000 records/day), and is contributing to an ongoing event-driven workstream using Java, Spring Boot, and Apache Kafka supporting a high-throughput pipeline (2M+ daily transactions) in a cloud-native Azure AKS environment. He also added AI-powered anomaly detection to the data pipeline, wrote a comprehensive PyTest suite, and set up Docker/Jenkins CI/CD.",
    orion:
